...
1. **Generate All Subsequences** – Basic recursion or backtracking
2. **Count Subsequences with Specific Property** – e.g. sum = target
3. **Longest Increasing Subsequence (LIS)** – Classic DP problem
4. **Subsequences with K elements** – Use recursion with element count
5. **Print all subsequences with sum = K** – Variation of subset sum
...
> 💡 **Key Insight:** This is the same binary decision tree structure used in `PowerSet.java` (Approach 2) — at each step, branch into **include** vs **exclude**, and the leaf nodes represent all 2ⁿ subsequences.
...
> ⚠️ Generating all subsequences has **exponential complexity**. Use it wisely for small `n` (usually ≤ 20).
